...
/* ============================================================
   WORKFLOW · CONSTELLATION renderer — markup shell
   A ✦ eyebrow + readout, a full-bleed starfield + connect-the-stars
   schematic (built imperatively by useConstellationDriver, including
   the SVG-text star names), and a bottom-left detail placard that
   crossfades per active step.
   ============================================================ */
...
import { useConstellationDriver } from '../useConstellationDriver';
import { renderCopy } from '../shared/renderCopy';
import styles from '../Constellation.module.css';

export function ConstellationWorkflow({ variant }: { variant: WorkflowVariant }) {
  ...
  useConstellationDriver(sectionRef, { layout, accents, reducedMotion });

  const className = [styles.constellation, reducedMotion ? styles.isStatic : '']
    .filter(Boolean)
    .join(' ');
  ...
  return (
    <section ref={sectionRef} className={className} id="workflow" data-wf>
      <div className={styles.viewport} data-viewport>
        ...
        {/* carries the step names to the imperative SVG-text labels */}
